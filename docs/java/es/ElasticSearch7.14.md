...
#3、文档的操作
  #3.1、文档的添加
    ...
    POST /products/_doc/2
    {
      "id": 2,
        ...
        "description": "小鱼饼干,真好吃!!"
    }
    #3.1.2、自动id添加文档 WhckLoIBAEVlAm1CKSOC
      POST /products/_doc
      {
        "title": "小鱼饼干",
        "price": "6.7",
  ...
  #3.2、文档的查询
    #3.2.1、根据id查询文档
      ...
      GET /products/_doc/WhckLoIBAEVlAm1CKSOC
    #3.2.2、查询所有文档
      GET /products/_search
      
  #3.3、文档的删除
    DELETE /products/_doc/WxcmLoIBAEVlAm1CJCNx
  #3.4、文档的更新
    #3.4.1、先删除原始文档,在将更新文档以新的内容插入
      PUT /products/_doc/WhckLoIBAEVlAm1CKSOC
      {
        "title": "小鱼鱼饼干"
      }
    #3.4.2、可以将数据原始内容保存,并在此基础上更新
      POST /products/_doc/WxcmLoIBAEVlAm1CJCNx/_update
      {
        "doc": {
          "title": "小鱼鱼饼干"
        }
  ...
  #3.5、文档的批量操作
    #3.5.1、批量索引两条文档
      ...
      {"index": {"_id": 3}}
      {"id": 3,"title":"鱼豆腐","price":20.00,"created_at":"2022-09-15","description":"鱼豆腐,真好吃"}
      {"index": {"_id": 4}}
      {"id": 4,"title":"臭豆腐","price":10.00,"created_at":"2022-09-15","description":"臭豆腐,真好吃"}
    #3.5.2、更新文档时同时删除文档
      POST /products/_doc/_bulk
      {"update": {"_id": 3}}
        {"doc": {"title": "中国鱼豆腐"}}
...
#4、高级查询
  ...
  #4.2 关键字查询 trem
  # 对于不分词的keyword 要全部搜索，对于分词的要安装分词规范进行搜索
  #注：es中的中文分词器是单字分词，英文是单个单词分词
  #注：es 中只有text类型会分词，keyword，integer，date，double，float等都不进行分词
    GET /products/_search
    {
      "query": {
        "term": {
  ...
  #4.5、通配符查询 wildcard ?匹配一个字符 *匹配任意个字符
    GET /products/_search
    {
      "query": {
        "wildcard": {
  ...
  #4.7、模糊查询 fuzzy 最大模糊错误 必须在0-2之间
  #搜索关键词长度为 2 不允许存在模糊
  #搜索关键词长度为3-5 允许一次模糊
  #搜索关键词长度大于5 允许最大2模糊
    GET /products/_search
  ...
  #4.9、多字段查询 multi_match
  #字段类型分词,将查询条件分词之后进行查询改字段
  ...
  #4.12、自定义标签高亮查询 post_tags pre_tags require_field_match
    GET /products/_search
    {
      "query": {
        "term": {
  ...
  #4.15、指定字段排序 sort 指定排序方式时es上的 _score就会失效
    GET /products/_search
    {
      "query": {
        "match_all": {}
